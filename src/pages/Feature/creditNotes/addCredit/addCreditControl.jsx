...
import { successToast, customerrorToast, credit_note, dropdown_api } from '../../../../core/core-index';
...
const dicountEditForm = yup
  .object({
    discount: yup
      .number()
      .test((value) => typeof value === 'number' && !/[eE+-]/.test(value.toString()))
...
const AddCreditNotesComponentController = (props) => {
  ...
  const getBankList = async () => {
    ...
    const newBankList = bankList?.data?.map((item) => {
      console.log(item, '--');
      return { id: item.userId, text: item?.name, name: item?.name };
    });
  ...
  const onSubmit = async (data) => {
    ...
    for (let i = 0; i < finalArray.length; i++) {
      ...
      formData.append(`items[${i}][discountValue]`, finalArray[i]?.discountAmount);
    }
    formData.append('customerId', data?.customerId?.value ? data?.customerId?.value : data?.customerId);
    ...
    formData.append('termsAndCondition', data?.termsAndCondition == undefined ? '' : data?.termsAndCondition);
    ...
    if (finalArray?.length > 0) {
      const checkQuantity = finalArray?.some((item) => item?.alertQuantity === 0);
      if (checkQuantity) {
  ...
  const afterModalSubmit = (modalData) => {
    ...
    let checkIndex = Stringified?.findIndex((item) => item?.id == modalData?.id);
  ...
  const SelectedList = (value) => {
    ...
    let removeFromDropDown = productOptionRemove?.findIndex((ele) => ele.id == value);
    productOptionRemove?.splice(removeFromDropDown, 1);

    setProductOption(productOptionRemove);
    try {
      setProductService(selectList?._id ? [...productService, selectList] : [...productService]);
      setRoundOffAction(!roundOffAction);
    } catch (error) {
      return false;
    }
  ...
  const calculateDiscount = () => {
    ...
    let sum = productService.reduce(function (accumulator, curValue) {
      return (parseFloat(accumulator) + parseFloat(calculateDiscountAmount(curValue))).toFixed(2);
    }, initialValue);
    return sum;
  };

  const calculateDiscountAmount = (record) => {
    var discountQuantity = productOptionCopy.find((ele) => ele.id == record?._id)?.quantity;
    let disQ = discountQuantity ? parseInt(discountQuantity) : 0;
    return record?.discountType == 2
      ? `${((disQ * record?.sellingPrice * record?.discountValue) / 100).toFixed(2)} `
      : disQ === 0
      ? '0.00'
  ...
  const calculateRate = (record) => {
    var Quantity = productOptionCopy.find((ele) => ele.id == record?._id)?.quantity;
  ...
  const calculateAmount = (record) => {
    var countFinal = productOptionCopy.find((ele) => ele.id == record?._id)?.quantity;

    var countTaxFinal = productServiceCopy.find((ele) => ele._id == record?._id)?.tax?.taxRate;

    let discountFinal =
      record?.discountType == 2
        ? `${(((countFinal ? countFinal : 0) * record?.sellingPrice * record?.discountValue) / 100).toFixed(2)} `
        : countFinal === 0
        ? '0.00'
        : record?.discountValue?.toFixed(2);

    let SumFinal = (countFinal ? countFinal : 0) * record?.sellingPrice - discountFinal;
  ...
  const calculateTaxAmount = (record) => {
    var countQuantity = productOptionCopy.find((ele) => ele.id == record?._id)?.quantity;
    var countTax = productServiceCopy.find((ele) => ele._id == record?._id)?.tax?.taxRate;
    let discountTax =
      record?.discountType == 2
        ? `${(((countQuantity ? countQuantity : 0) * record?.sellingPrice * record?.discountValue) / 100).toFixed(2)} `
        : countQuantity === 0
        ? '0.00'
        : record?.discountValue?.toFixed(2);

    let SumTax = (countQuantity ? countQuantity : 0) * record?.sellingPrice - discountTax;
  ...
  const calculateFinalTotal = () => {
    let finalTotal = parseFloat(calculateSum()) + parseFloat(calculateTax()) - parseFloat(calculateDiscount());
