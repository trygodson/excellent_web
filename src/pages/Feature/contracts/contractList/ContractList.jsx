...
const ListContract = () => {
  ...
  const editModal = (data) => {
    let paymentmethodobj = paymentMethodsData.find((obj) => obj.value == data?.payment_method);
    resetField('notes');
    ...
    setValue('payment_method', paymentmethodobj || { label: 'Cash', value: 'Cash' });
  };
  ...
  const handleKeyDown = (event) => {
    ...
    if (isBackspaceOrDelete && selectionStart === selectionEnd && selectionStart > 0) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Link to={view || admin ? { pathname: `${'/view-invoice'}/${record._id}` } : '#'} className="invoice-link">
    ...
    },
    {
      ...
      render: (text, record) => (
        <h2 className="table-avatar">
          <Link to={{ pathname: `${'/view-customer'}/${record?.customerId?._id}` }} className="avatar avatar-sm me-2">
            {record.renderImage}
            {record?.customerId?.image ? (
              ...
              <img className="aavatar avatar-sm me-2 avatar-img rounded-circle" src={alterUserimg} alt="User Image" />
            )}
          </Link>
          <Link to={{ pathname: `${'/view-customer'}/${record?.customerId?._id}` }}>
            {record?.customerId?.name ? record?.customerId?.name : `Deleted Customer`}{' '}
            {record?.customerId?.phone && <span>{record?.customerId?.phone}</span>}
          </Link>
        </h2>
      ),
    },
    ...
    },
    {
      ...
      render: (text) => (
        <div>
          {text === 'REFUND' && <span className="badge bg-info-lights">{convertFirstLetterToCapital(text)}</span>}
          {text === 'SENT' && <span className="badge bg-info-lights">{convertFirstLetterToCapital(text)}</span>}
          {text === 'UNPAID' && (
            <span className="badge bg-light-gray text-secondary">{convertFirstLetterToCapital(text)}</span>
          )}
          {text === 'PARTIALLY_PAID' && <span className="badge bg-primary-light">Partially Paid</span>}
          {text === 'CANCELLED' && <span className="badge bg-danger-light">{convertFirstLetterToCapital(text)}</span>}
          {text === 'OVERDUE' && <span className="badge bg-danger-light">{convertFirstLetterToCapital(text)}</span>}
          {text === 'PAID' && <span className="badge bg-success-light">{convertFirstLetterToCapital(text)}</span>}
          {text === 'DRAFTED' && <span className="badge bg-warning">{convertFirstLetterToCapital(text)}</span>}
        </div>
      ),
    },
    (create || update || admin) && {
          ...
          <div className="d-flex align-items-center customer-details">
            <div className="dropdown dropdown-action">
              ...
              <div className="dropdown-menu dropdown-menu-end quatation-dropdown invoice-dropdown-menu">
                {record?.status != 'PAID' && record?.payment_method != 'Online' && (create || update || admin) && (
                  <Link
                ...
                {(update || admin) && record?.status != 'PAID' && record?.status != 'PARTIALLY_PAID' && (
                  <Link className="dropdown-item" to={{ pathname: `${'/edit-invoice'}/${record._id}` }}>
  ...
  return (
    <>
                  ...
                  <div className="invoices-tabs">
                    <ul>
                      ...
                      {/* <li>
                        <Link
                          to={'/invoice-list'}
                          className={activeTab == 'PARTIALLY_PAID' ? 'active' : ''}
            ...
            <div className="modal-body">
              ...
              <div className="modal-btn delete-action">
                <div className="row">
                  ...
                  <div className="col-6">
                    <button type="submit" data-bs-dismiss="modal" className="w-100 btn btn-primary paid-cancel-btn">
                      Cancel
                    </button>
                  </div>
                </div>
            ...
            <div className="modal-body">
              ...
              <div className="modal-btn delete-action">
                <div className="row">
                  ...
                  <div className="col-6">
                    <button data-bs-dismiss="modal" className="w-100 btn btn-primary paid-cancel-btn">
                      Cancel
                    </button>
                  </div>
                </div>
      ...
      <div className="modal custom-modal fade" id="send_payment_modal" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-md">
          <div className="modal-content">
            <div className="modal-body">
              <div className="form-header">
      ...
      <div className="modal custom-modal fade" id="requringStatus_modal" role="dialog">
        <div className="modal-dialog modal-dialog-centered modal-md">
          <div className="modal-content">
            <div className="modal-body">
              <div className="form-header">
                  ...
                  <div className="col-lg-6 col-md-6">
                    <div className="form-group input_text">
                      ...
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${errors?.invoiceAmount ? 'error-input' : ''}`}
                            type="text"
                            value={value || ''}
                            onChange={onChange}
                            readOnly={true}
                  ...
                  <div className="col-lg-6 col-md-6">
                    <div className="form-group input_text">
                      ...
                      <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                          <input
                            className={`form-control ${errors?.balanceAmount ? 'error-input' : ''}`}
                            type="text"
                            value={value || ''}
                            onChange={onChange}
                            readOnly={true}
                    ...
                    <div className="form-group input_text">
                      ...
                      <Controller
                        ...
                        render={({ field: { onChange } }) => (
                          <DatePicker
                            className={`datetimepicker w-100 form-control ${errors?.received_on ? 'error-input' : ''}`}
                            selected={receivedon}
                  ...
                  <div className="col-lg-12 col-md-12">
                    <div className="form-group input_text">
                      ...
                      <Controller
                        ...
                        render={({ field: { onChange } }) => (
                          <input
                            className={`form-control ${errors?.amount ? 'error-input' : ''}`}
                            type="text"
                            name="amount"
                            value={amountValue}
                            onChange={onChange}
