...
const Header = memo((props) => {
  const { loggedin, setloggedin, setnotificationFlag, notificationFlag } = useContext(AuthContext);
  ...
  useEffect(() => {
    const getDatas = async () => {
      try {
        const response = await getData(`/notification/listNotification?skip=&limit=5`, false);
        if (response?.data) {
  ...
  return (
    <>
      <Helmet>
        <title>Excellent Contructions</title>
        <meta name="description" content="Excellent Contructions" />
        <link rel="icon" href={companyData?.favicon ? companyData?.favicon : AlterFavIcon}></link>
      </Helmet>
      <div className="header header-one">
        ...
        <Link className="mobile_btn" id="mobile_btn" onClick={() => onMenuClik()}>
          <i className="fas fa-bars" />
        </Link>
        {/* /Mobile Menu Toggle */}
        {/* Header Menu */}
        <ul className="nav nav-tabs user-menu">
          <li className="nav-item dropdown  flag-nav dropdown-heads" onClick={() => setnotificationFlag(false)}>
            <Link className="nav-link bell-icon" data-bs-toggle="dropdown" to="#" role="button">
              {notificationFlag ? (
                <Badge count={notificationdata.length || 0} overflowCount={4} color="hwb(205 6% 9%)">
            ...
            <div className="dropdown-menu notifications">
                  ...
                  {notificationdata.length > 0 ? (
                          ...
                          <div className="media d-flex">
                            ...
                            <div className="media-body">
                              <p className="noti-details">
                                <span className="noti-title">{data?.title}</span>
                              </p>
                              <p className="noti-time">
                                <span className="notification-time">{data?.body}</span>
                              </p>
                              <p className="noti-time">
                    ...
                    <>
                      <p className="no_notifications">
                        <span className="no_notifications_text">No Notifications</span>
                      </p>
                    </>
                  )}
                </ul>
              ...
              <div className="topnav-dropdown-footer">
                {notificationdata.length > 0 && (
                  <Link to="#" className="clear-noti" onClick={() => clearNotifications()}>
                    {' '}
                    Clear All
                  </Link>
                )}
          ...
          <li className="nav-item dropdown">
            <Link to="#" className="user-link  nav-link" data-bs-toggle="dropdown">
              ...
              <span className="user-content">
                <span className="user-details">{isAdmin ? 'Super Admin' : capitalizedRole}</span>
            ...
            <div className="dropdown-menu menu-drop-user">
              <div className="profilemenu">
                ...
                <div className="subscription-logout">
                  <ul>
                    <li className="pb-0">
                      <Link className="dropdown-item" to="/login" onClick={() => logoutPage()}>
                        Log Out
                      </Link>
                    </li>
                  </ul>
